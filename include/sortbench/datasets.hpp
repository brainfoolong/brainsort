...
template <> struct KeyGen<Item> {
    using key = int32_t;
    static key random(std::mt19937_64& rng) { return std::uniform_int_distribution<int32_t>(INT32_MIN, INT32_MAX)(rng); }
...
template <> struct KeyGen<DblItem> {
    using key = double;
    static key random(std::mt19937_64& rng) { return std::uniform_real_distribution<double>(-1e6, 1e6)(rng); }
    static key from_int(int64_t v) { return static_cast<double>(v) * 0.25; }
    static key prefixed(std::mt19937_64& rng) { return 1e9 + std::uniform_real_distribution<double>(0, 1e6)(rng); }
...
inline std::vector<typename KeyGen<T>::key> generate_keys(const std::string& name, size_t n, uint64_t seed) {
    ...
    if (name == "random") {
        ...
        if (n > 1) {
            ...
            for (size_t s = 0; s < swaps; ++s) std::swap(v[d(rng)], v[d(rng)]);
        }
        ...
        std::uniform_int_distribution<size_t>  len(16, 2000);
        std::uniform_int_distribution<int64_t> step(0, 10);
        size_t i = 0;
        while (i < n) {
            size_t  l   = std::min(len(rng), n - i);
            int64_t key = std::uniform_int_distribution<int64_t>(-100000, 100000)(rng);
            for (size_t k = 0; k < l; ++k) { v[i + k] = G::from_int(key); key += step(rng); }
            i += l;
        }
