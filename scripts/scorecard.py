...
fewest comparisons and the least scratch memory against the other sorts of
the pool, every one of them stable upstream code. The Rust block does the
same against the Rust sorts as shipped on the two of those metrics that can
be counted without their source (comparisons and scratch memory) plus the
compare flips. These numbers are deterministic, so the same tables come out
on every machine; the timing lives on the website.
"""
import csv
import os
import re
...
def wins(cells, metric):
    won = total = 0
    for cell in cells.values():
        if CANDIDATE not in cell:
            continue
        opp = [float(r[metric]) for a, r in cell.items() if a != CANDIDATE]
        if not opp:
            continue
        total += 1
...
def block(cells, metrics, begin, end, what):
    lines = [begin,
             f"| deterministic, n = {N:,}, {len(cells)} cells | against the other {what}s |",
             "|---|---:|"]
    for metric, label in metrics:
        ws, ts = wins(cells, metric)
        lines.append(f"| {label}: brainsort lowest or tied | **{ws} / {ts}** |")
    lines.append(end)
    return "\n".join(lines)
