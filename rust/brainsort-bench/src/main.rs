...
fn run_bench(reps: usize, max_n: usize, host: &str, out_path: &str) {
    ...
    let head = format!(
        "{os} {arch}, {comp}, {cpu}, code {code}, commit {commit}, {when}{host_part}. brainsort::sort on a plain Vec against slice::sort, slice::sort_by_cached_key and glidesort; median of {reps} runs, wall ms per sort (below 100,000 elements, ceil(100,000/n) copies are sorted back to back). Generated by brainsort-bench --bench."
    );
    let fields = [
        ("host", json_str(host)),
        ("os", json_str(os)),
