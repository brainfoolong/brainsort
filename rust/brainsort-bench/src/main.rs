...
//!   brainsort-bench --rust-counts [--sizes 10,100,...] [--out FILE]
...
fn main() {
    ...
    let mut sizes: Vec<usize> = vec![10, 100, 1_000, 10_000, 100_000, 1_000_000];
...
fn run_bench(reps: usize, max_n: usize, host: &str, out_path: &str) {
    use timing::*;
    let sizes: Vec<usize> = [10usize, 100, 100_000, 1_000_000, 10_000_000].into_iter().filter(|&n| n <= max_n).collect();
    let reps = reps.max(1);
    ...
    let head = format!(
        "{os} {arch}, {comp}, {cpu}, code {code}, commit {commit}, {when}{host_part}. brainsort::sort on a plain Vec against the standard library's stable and unstable sorts and the radix crates radsort, voracious_radix_sort and rdst; median of {reps} runs, wall ms per sort (below 100,000 elements, ceil(100,000/n) copies are sorted back to back). Generated by brainsort-bench --bench."
    );
    let fields = [
        ("host", json_str(host)),
        ("os", json_str(os)),
